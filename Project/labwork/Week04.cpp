...
void VulkanBase::createImageViews()
{
	...
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		swapChainImageViews[i]
			= Texture::CreateImageView(swapChainImages[i],
...
VkFormat VulkanBase::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
	for (VkFormat format : candidates) {
		...
		vkGetPhysicalDeviceFormatProperties(Globals::physicalDevice(), format, &props);
...
{
	Texture::CreateImage(swapChainExtent.width,
		...
		m_ColorImage,
		m_ColorImageMemory);
	m_ColorImageView = Texture::CreateImageView(m_ColorImage,
		swapChainImageFormat,
		VK_IMAGE_ASPECT_COLOR_BIT
		//,1
	);
...
{
	vkDestroyImageView(Globals::device(), m_ColorImageView, nullptr);
	vkDestroyImage(Globals::device(), m_ColorImage, nullptr);
	vkFreeMemory(Globals::device(), m_ColorImageMemory, nullptr);

	vkDestroyImageView(Globals::device(), m_DepthImageView, nullptr);
	vkDestroyImage(Globals::device(), m_DepthImage, nullptr);
	vkFreeMemory(Globals::device(), m_DepthImageMemory, nullptr);

	for (const auto framebuffer : m_swapChainFramebuffers)
	{
		vkDestroyFramebuffer(Globals::device(), framebuffer, nullptr);
	}

	for (const auto imageView : swapChainImageViews)
	{
		vkDestroyImageView(Globals::device(), imageView, nullptr);
	}

	vkDestroySwapchainKHR(Globals::device(), swapChain, nullptr);
}
...
void VulkanBase::createFrameBuffers()
{
	...
	for (size_t i = 0; i < swapChainImageViews.size(); i++)
	{
		std::array<VkImageView, 3> attachments
			= { m_ColorImageView, m_DepthImageView, swapChainImageViews[i] };

		VkFramebufferCreateInfo framebufferInfo{};
		framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
		framebufferInfo.renderPass = Globals::renderPass();
		...
		if (vkCreateFramebuffer(Globals::device(),
			&framebufferInfo,
			nullptr,
